...
var fs_1 = require("fs");
var x, y, steps, value, wire1, wire2;
var plot, intersections;
var input = (0, fs_1.readFileSync)('input', 'utf-8').split('\n');
function check_plots(input) {
...
function find_distance(str) {
    var values = str.split(',');
    values = values.map(Number);
    return Math.abs(values[0]) + Math.abs(values[1]);
}
function get_outputs(input) {
    input.forEach(function (val, key) {
        var new_distance = find_distance(key);
